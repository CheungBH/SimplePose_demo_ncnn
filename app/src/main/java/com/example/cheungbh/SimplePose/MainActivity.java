package com.example.cheungbh.SimplePose;
...
import com.example.cheungbh.SimplePose.SimplePose;
...
public class MainActivity extends AppCompatActivity {
    ...
    private int[] ddims = {1, 3, 192, 256}; //这里的维度的值要和train model的input 一一对应
    private int model_index = 1;
    private List<String> resultLabel = new ArrayList<>();
    private SimplePose simplepose = new SimplePose(); //java接口实例化　下面直接利用java函数调用NDK c++函数
    ...
    private void initMobileNetSSD() throws IOException {
        ...
        byte[] bin = null;
        {
            ...
            InputStream assetsInputStream = getAssets().open("pose.param");// param：  网络结构文件
        ...
        }
        {
            ...
            InputStream assetsInputStream = getAssets().open("pose.bin");//bin：   model文件
            int available = assetsInputStream.available();
        ...
        load_result = simplepose.Init(param, bin);// 再将文件传入java的NDK接口(c++ 代码中的init接口 )
        Log.d("load model", "Simplepose_load_model_result:" + load_result);
    }
